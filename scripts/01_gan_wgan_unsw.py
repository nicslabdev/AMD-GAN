"""
WGAN-GP Training for Class-Specific Synthetic Network Traffic Generation (UNSW-NB15)

Trains adaptive WGAN-GP models for each traffic class with configuration
...
class PreprocesadorUNSW :
    """Preprocesador específico para UNSW-NB15."""
    ...
    def preparar_df_base (df :pd .DataFrame )->pd .DataFrame :
        """Prepara el DataFrame base con IPs expandidas y filtrado de clases."""
        df =df .copy ()
        df .columns =df .columns .str .strip ()
...
def main ():
    ...
    description ='Entrenamiento WGAN-GP para UNSW-NB15',
    ...
    print ("="*80 )
    print ("ENTRENAMIENTO WGAN-GP - UNSW-NB15")
    print ("="*80 )


    print ("\n[1] Cargando dataset UNSW-NB15...")
    prep_global =PreprocesadorUNSW (DATASET_PATH )
    df_raw =prep_global .cargar ()
    df_base =PreprocesadorUNSW .preparar_df_base (df_raw )
    ...
    for clase ,count in class_counts .items ():
        ...
        print (f"  {marker }{clase :<20}: {count :>10,} muestras")
    print ("-"*50 )
    ...
    for class_name in classes_to_train :
        ...
        prep_cls =PreprocesadorUNSW (DATASET_PATH )
