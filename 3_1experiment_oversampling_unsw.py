"""
Script: Comparación de Técnicas de Oversampling para UNSW-NB15

Compara diferentes técnicas de oversampling para balancear el dataset UNSW-NB15:
  1. Original (sin oversampling) - baseline
  2. GAN (WGAN-GP entrenada en UNSW-NB15)
  3. SMOTE
  4. ADASYN
  5. BorderlineSMOTE
...
Usage:
    python experiment_oversampling_unsw.py
    python experiment_oversampling_unsw.py --skip-gan
    python experiment_oversampling_unsw.py --max-samples 50000
"""
...
OUTPUT_DIR = '<PATH_TO_RESULTS_OVERSAMPLING_UNSW>'
RANDOM_STATE = 42
LATENT_DIM = 100
LABEL_COLUMN = 'Label'

# Classs a usar en el experimento (descartamos las de <500 samples)
...
def print_header():
    print("=" * 100)
    print("  EXPERIMENTO: COMPARACIÓN DE TÉCNICAS DE OVERSAMPLING - UNSW-NB15")
...
def mostrar_distribucion(y, label_encoder, titulo="Distribution"):
    ...
    for label_idx in sorted(counter.keys()):
        ...
        pct = (count / total) * 100
        print(f"    {class:<20}: {count:>10,} ({pct:>5.1f}%)")
...
def oversample_gan(X_train, y_train, label_encoder, target_count):
    ...
    print("\n  [GAN] Aplicando oversampling con WGAN-GP UNSW-NB15...")
...
def generar_summary(results, label_encoder, output_dir):
    ...
    summary = []
    summary.append("=" * 120)
    summary.append(" " * 20 + "COMPARACIÓN DE TÉCNICAS DE OVERSAMPLING - UNSW-NB15")
    ...
    header_line = f"{'Class':<20} | " + " | ".join([f"{t:>12}" for t in tecnicas_nombres])
    summary.append("\n" + "-" * len(header_line))
    ...
    for class in label_encoder.classes_:
        ...
        summary.append(f"{class:<20} | " + " | ".join(valores))

    # Conclusión
    summary.append("\n\n" + "=" * 120)
    summary.append("4. CONCLUSIONES")
    summary.append("=" * 120)
...
def main():
    parser = argparse.ArgumentParser(
        description='Oversampling technique comparison para UNSW-NB15'
    )
